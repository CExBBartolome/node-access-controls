...
function AccessControlList(conf) {
  ...
  var hasSaveActions = this._actions['save'] ||
    this._actions['save_new'] ||
	 this._actions['save_existing'];

  if(conf.control === 'filter' && hasSaveActions) {
	  throw new Error('save action is not allowed for filter control type')
  }
